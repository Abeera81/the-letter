...
2. **The Span Gate** ([`src/lib/spanGate.ts`](src/lib/spanGate.ts)) — plain TypeScript,
   **no model in it**, confirms each quote genuinely appears in the source. Anything it
   can't find is **deleted** before it is ever spoken.
